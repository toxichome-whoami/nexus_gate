    ...
    for field in fields:
        if not mapping.get(field):
            raise NexusGateException(
                ErrorCodes.INPUT_SCHEMA_INVALID, f"'{field}' is required", 400
            )
...
def _extract_remote_databases(alias: str, db_list: dict, local_dbs: dict) -> None:
    ...
    for db_name, db_info in db_list.items():
        ...
        local_dbs[f"{alias}_{db_name}"] = {
            ...
            "status": info_dict.get("status", db_info)
            if not isinstance(db_info, dict)
            else info_dict.get("status", "unknown"),
    ...
    static_keys = [
        {
            ...
            "fs_scope": cfg.fs_scope,
            "rate_limit_override": cfg.rate_limit_override,
            "full_admin": cfg.full_admin,
        }
    ...
    db_keys = [
        {
            ...
            "source": "sqlite",
            "mode": data["mode"],
            "db_scope": data["db_scope"],
            "fs_scope": data["fs_scope"],
...
async def create_api_key(
    request: Request, body: dict = Body(...), auth=Depends(require_admin)
):
...
async def revoke_api_key(
    request: Request, key_name: str = Path(...), auth=Depends(require_admin)
):
    if key_name == auth.api_key_name:
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, "You cannot revoke your active API key", 400
        )
    ...
    if not deleted and not banned:
        raise NexusGateException(
            ErrorCodes.AUTH_INVALID_KEY, f"Key '{key_name}' not found", 404
        )
...
async def update_api_key(
    ...
    updates = {
        ...
        if k in ["mode", "db_scope", "fs_scope", "rate_limit_override"]
    }
    if not updates:
        raise NexusGateException(
            ErrorCodes.INPUT_SCHEMA_INVALID, "No valid parameters provided", 400
        )

    if not await SecurityStorage.update_api_key(name, updates):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"Dynamic key '{name}' not found", 404
        )
...
async def ban_ip(request: Request, body: dict = Body(...), auth=Depends(require_admin)):
    ...
    if not isinstance(ip, str):
        raise NexusGateException(
            ErrorCodes.INPUT_SCHEMA_INVALID, "IP must be a valid string", 400
        )

    await BanList.ban_ip(
        ip,
        reason=body.get("reason", "Manual ban via admin API"),
        duration_seconds=body.get("duration_seconds"),
    )
    return success_response(
        request, {"banned_ip": ip, "reason": body.get("reason", "Manual ban")}
    )
...
async def unban_ip(request: Request, ip: str = Path(...), auth=Depends(require_admin)):
    if not await BanList.unban_ip(ip):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"IP '{ip}' is not banned", 404
        )
...
async def ban_key(
    ...
    if key_name == auth.api_key_name:
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID,
            "Lockout protection: cannot ban active key",
            400,
        )

    await BanList.ban_key(
        key_name,
        reason=body.get("reason", "Manual array"),
        duration_seconds=body.get("duration_seconds"),
    )
    return success_response(
        request, {"banned_key": key_name, "reason": body.get("reason", "Manual array")}
    )
...
async def unban_key(
    ...
    if not await BanList.unban_key(key_name):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"Key '{key_name}' is not banned", 404
        )
...
async def reset_circuit_breaker(
    request: Request, key: str = Path(...), auth=Depends(require_admin)
):
...
async def create_database(
    ...
    if not isinstance(name, str):
        raise NexusGateException(
            ErrorCodes.INPUT_SCHEMA_INVALID, "Database name must be a valid string", 400
        )
    ...
    return success_response(
        request, {"created_database": name, "note": "Database dynamically attached."}
    )


@router.delete("/databases/{name}")
async def delete_database(
    request: Request, name: str = Path(...), auth=Depends(require_admin)
):
    if not await SecurityStorage.delete_database(name):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"Dynamic Database '{name}' not found", 404
        )
...
async def update_database(
    ...
    if any(blocked in body for blocked in ["query_whitelist", "query_blacklist"]):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID,
            "Schema validations are config.toml only.",
            400,
        )
    ...
    if not updates:
        raise NexusGateException(
            ErrorCodes.INPUT_SCHEMA_INVALID, "No updatable fields", 400
        )

    if not await SecurityStorage.update_database(name, updates):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"Database '{name}' missing", 404
        )
...
async def delete_webhook(
    ...
    if not await SecurityStorage.delete_webhook(name):
        raise NexusGateException(
            ErrorCodes.INPUT_VALUE_INVALID, f"Dynamic hook '{name}' missing", 404
        )
...
async def update_webhook(
    ...
    if not updates or not await SecurityStorage.update_webhook(name, updates):
