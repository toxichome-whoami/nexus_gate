...
def _build_base_headers(inline: bool, mime_type: str, filename: str, etag: str) -> dict:
    ...
    headers = {
        ...
        "Cache-Control": "public, max-age=31536000, immutable" if mime_type.startswith(("image/", "font/", "audio/", "video/")) else "public, max-age=3600"
    }
    return headers
...
def serve_file(path: str, inline: bool = False, request_headers: Optional[dict] = None) -> Response:
    ...
    base_headers = _build_base_headers(inline, get_mime_type(path), os.path.basename(path), etag)
    byte_range = _parse_range_header(req_headers.get("range", ""), file_stat.st_size)

    if req_headers.get("range", "") and byte_range is None:
        return Response(status_code=416, headers={"Content-Range": f"bytes */{file_stat.st_size}", **base_headers})

    if byte_range:
        base_headers.update({"Content-Range": f"bytes {byte_range[0]}-{byte_range[1]}/{file_stat.st_size}", "Content-Length": str(byte_range[1] - byte_range[0] + 1)})
        return StreamingResponse(_file_range_streamer(path, byte_range[0], byte_range[1]), status_code=206, media_type=get_mime_type(path), headers=base_headers)

    base_headers["Content-Length"] = str(file_stat.st_size)
    return StreamingResponse(_file_streamer(path), status_code=200, media_type=get_mime_type(path), headers=base_headers)
