...
def _enforce_user_mode(expr: Any, user_mode: str) -> None:
...
def _enforce_query_policy(
    expr: Any, db_config: DatabaseDefConfig, query_type: str
) -> None:
...
def _extract_target_table(expr: Any) -> str:
...
def validate_query(
    sql: str, db_config: DatabaseDefConfig, user_mode: str
) -> tuple[str, str, str]:
