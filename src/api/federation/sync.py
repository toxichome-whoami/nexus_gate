...
async def _poll_single_remote_node(
    alias: str, srv_config, state: FederationState
) -> None:
    ...
    try:
        ...
        logger.warning(
            "Failed to sync with federated server", alias=alias, error=str(net_error)
        )
        state.servers[alias] = {"status": "down", "error": str(net_error)}


async def _execute_synchronization_cycle(config: Any, state: FederationState) -> None:
