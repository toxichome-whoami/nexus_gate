...
async def _compile_full_schema(uri: str, alias: str) -> TextResourceContents:
    ...
    if not active_engine:
        return build_error_text_resource(
            uri, f"Database '{alias}' is offline or disconnected."
        )
    ...
    for table in database_tables:
        ...
        markdown_lines.extend(
            f"  - {ResultFormatter.format_column_line(c)}" for c in table_columns
        )
...
async def _read_database_schema(uri: str) -> list[TextResourceContents]:
    ...
    if not target_database_alias:
        return [
            build_error_text_resource(uri, "Invalid database Resource URI structure.")
        ]
...
def register_database_resources() -> None:
    ...
    mcp_resource_registry.register_reader_prefix(
        URI_PROTOCOL_PREFIX, _read_database_schema
    )
