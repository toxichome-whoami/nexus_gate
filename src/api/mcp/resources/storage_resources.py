...
async def _compile_storage_info(uri: str, alias: str) -> TextResourceContents:
    ...
    if not storage_config:
        return build_error_text_resource(uri, f"Storage alias '{alias}' is completely invalid.")
...
async def _read_storage_info(uri: str) -> list[TextResourceContents]:
    ...
    if not target_storage_alias:
        return [build_error_text_resource(uri, "Encountered broken storage Resource URI structure.")]
...
def register_storage_resources() -> None:
    ...
    mcp_resource_registry.register_reader_prefix(URI_PROTOCOL_PREFIX, _read_storage_info)
