...
async def _handle_penalty_application(client: redis.Redis, limits_key: str, penalty_key: str, window: int, penalty_cooldown: int) -> None:
    ...
    async def check_rate_limit(cls, limits_key: str, window: int, limit: int, penalty_key: str, burst: int, penalty_cooldown: int) -> tuple[bool, int]:
        ...
        try:
            violated, count = await client.eval(
                SLIDING_WINDOW_LUA_SCRIPT, 1, limits_key, 
                window_start, now, limit + burst, window
            )
            
            if violated == 1:
                await _handle_penalty_application(client, limits_key, penalty_key, window, penalty_cooldown)
                return True, count
            
            return False, count
            
        except Exception as script_error:
            logger.error("Redis rate limit LUA execution failed", error=str(script_error))
            return False, 0
