...
def _rotate_active_log(
    active_log: str, max_size_bytes: int, directory: str, prefix: str, today: str
):
...
def _garbage_collect_logs(directory: str, prefix: str, max_files: int):
    ...
    for target_file in old_files_to_delete:
        try:
            ...
            logger.error(
                "Failed to delete log file", file=target_file, error=str(delete_error)
            )
