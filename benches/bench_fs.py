...
async def worker(client: httpx.AsyncClient, url: str, headers: dict, stats: dict, semaphore: asyncio.Semaphore):
...
async def main():
    ...
    print(f"--- NexusGate FS Benchmark ---")
    print(f"Target: {url}")
    ...
    async with httpx.AsyncClient(limits=httpx.Limits(max_connections=concurrency)) as client:
        tasks = [worker(client, url, headers, stats, semaphore) for _ in range(total_requests)]
